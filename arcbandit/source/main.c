...
static PVOID PciPhysToVirt(ULONG Addr) {
	// OldWorld systems with bandit map to 0xF000_0000 area, so check that too 
	if (Addr > 0xF0000000) return (PVOID)Addr;
	return (PVOID) (((Addr) & ~0x90000000) | 0xC0000000);
}

//---------------------------------------------------------------------------------
void ARC_NORETURN FwMain(PHW_DESCRIPTION Desc) {
//---------------------------------------------------------------------------------
	//asm volatile("addis 30, 30, 0x3000; li 19, 10; bl BlitScreen");
	...
	// TODO: Determine when this does NOT hold true.
	if (*(ULONG*)PciPhysToVirt(Desc->GrandCentralStart + 0x1D000) != 0x0000FFFF) {
		FwEarlyPanic("PCI space not little endian!");
	}
