...
static bool PxipIsBusy(void) {
	if (s_PxiIsCuda) {
		return (s_PxiRegs->IFR & PXI_IE_SR) == 0;
	}
...
static void PxipNotifyMcuTipNoAck(bool Value) {
	UCHAR BufB = s_PxiRegs->BufB;
	MmioWrite8(&s_PxiRegs->BufB, Value ? BufB & ~PXI_PORT_TIP_CUDA : BufB | PXI_PORT_TIP_CUDA);
}
...
static void PxipWriteTxStart(UCHAR Data) {
	...
	/*printf("%02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\r\n",
		s_PxiRegs->BufB, s_PxiRegs->BufAH, s_PxiRegs->DirB, s_PxiRegs->DirA,
		s_PxiRegs->T1C, s_PxiRegs->T1CH, s_PxiRegs->T1L, s_PxiRegs->T1LH,
		s_PxiRegs->T2C, s_PxiRegs->T2CH, s_PxiRegs->ACR, s_PxiRegs->rPCR,
		s_PxiRegs->IFR, s_PxiRegs->IER, s_PxiRegs->BufA);*/
	//printf("wait 5\r\n");
	while (PxipIsBusy()) {}
}
...
void PxiInit(PVOID MmioBase, bool IsCuda) {
	...
	MmioWrite8(&s_PxiRegs->IER, (UCHAR)~(UCHAR)((UCHAR)PXI_IE_CB1 | (UCHAR)PXI_IE_SR | (UCHAR)PXI_IE_SET));
